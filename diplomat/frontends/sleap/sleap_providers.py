...
class PreProcessingLayer:
    ...
    def __init__(self, config: dict, **kwargs):
        self._config = config
        self._preprocess_config = _dict_get_path(
            self._config, ("data", "preprocessing"), {}
        )
        ...
        if is_grayscale:
            rgb_to_gray = OnnxOp(
                "Constant",
                value=onnx_np_helper.from_array(
        ...
        if p_c.get("resize_and_pad_to_target", False):
            ...
            if self._t_width is not None and self._t_height is not None:
                desired_size = OnnxOp(
                    "Constant",
                    value=onnx_np_helper.from_array(
                        np.array([self._t_height, self._t_width], dtype=np.int64)
                    ),
                )
                mdl = OnnxOp(
                    "Resize",
        ...
        if self._post_input_scale != 1.0:
            desired_scale = OnnxOp(
                ...
                value=onnx_np_helper.from_array(
                    np.array(
                        [self._post_input_scale, self._post_input_scale],
                        dtype=np.float32,
                    )
                ),
            )
        ...
        graph = to_onnx_graph_def(
            ...
            [
                mdl.to_var(
                    "OUTPUT",
                    TensorProto.FLOAT,
                    (None, None, None, 1 if is_grayscale else 3),
                )
            ],
        )
        self._preprocess_model = oh.make_model(
            graph, opset_imports=[onnx.OperatorSetIdProto(version=19)]
        )
        onnx.checker.check_model(self._preprocess_model)
        self._preprocess_runner = _onnx_model_to_inference_session(
            self._preprocess_model, **kwargs
        )

        for backbone in self._config["model"]["backbone"].values():
            if backbone is not None:
                # TODO: Calculate this properly from actual model...
                self._pad_to_stride = int(
                    backbone.get("max_stride", backbone.get("output_stride", 1))
                )
    ...
    def __call__(self, img: np.ndarray) -> Tuple[np.ndarray, int]:
        ...
        if self._t_width is not None and self._t_height is not None:
            downscaling /= min(
                self._t_height / img.shape[1], self._t_width / img.shape[2]
            )
...
def _resolve_heads(outputs, heads):
    return [outputs[h_idx] if (h_idx is not None) else None for h_idx in heads]
...
class BottomUpModelExtractor(SleapModelExtractor):
    ...
    def __init__(self, models: ConfigAndModels, refinement_kernel_size: int, **kwargs):
        ...
        if (
            offset_ref_name not in map(lambda x: x.name, self._model.graph.output)
            and refinement_kernel_size > 1
        ):
            self._refinement_kernel_size = refinement_kernel_size
            post_model = onnx.helper.make_model(
                _get_integral_offsets_model(
                    refinement_kernel_size, 1.0, "conf_maps", offset_ref_name
                ),
                ir_version=self._model.ir_version,
                opset_imports=[
                    onnx.OperatorSetIdProto(version=self._model.opset_import[0].version)
                ],
            )
    ...
    def extract(
        ...
        conf_map, offsets = _resolve_heads(
            self._model.run(None, {self._model.get_inputs()[0].name: data}), self._heads
        )
        ...
        return (_fix_conf_map(conf_map), offsets, first_scale_val * cmap_dscale)
...
class SingleInstanceModelExtractor(SleapModelExtractor):
    ...
    def __init__(self, models: ConfigAndModels, refinement_kernel_size: int, **kwargs):
        ...
        if (
            ...
            post_model = onnx.helper.make_model(
                _get_integral_offsets_model(
                    self._refinement_kernel_size, 1.0, "conf_maps", offset_ref_name
                ),
                ir_version=self._model.ir_version,
                opset_imports=[
                    onnx.OperatorSetIdProto(version=self._model.opset_import[0].version)
...
@resolve_lazy_imports
def _get_integral_offsets_model(
    kernel_size: int,
    stride: float,
    input_name: str = "conf_maps",
    output_name: str = "offsets",
):
    ...
    bc_comb_shape = OnnxOp(
        "Concat",
        OnnxOp("ReduceProd", split_shape[0], keepdims=1),
        split_shape[1],
        axis=0,
    )
    ...
    mdl = OnnxOp(
        ...
        OnnxOp(
            ...
            value=onnx.numpy_helper.from_array(
                _get_integral_offset_kernels(kernel_size, stride, np.float32)
            ),
        ),
        auto_pad="SAME_UPPER",
    )
    ...
    split_shape_new = OnnxOp(
        "Split", full_shape_new, OnnxOp("Constant", value_ints=[1, 3])
    )
    ...
    # Nan to zero...
    mdl = OnnxOp(
        "Where", OnnxOp("Equal", mdl, mdl), mdl, OnnxOp("Constant", value_floats=[0.0])
    )
    mdl = OnnxOp("Mul", mdl, OnnxOp("Constant", value_floats=[stride]))

    return to_onnx_graph_def(
        "IntegralOffsets", [mdl.to_var(output_name, FLOAT, (None, None, None, None, 2))]
    )
...
class PredictorExtractor:
    def __init__(self, configs: ConfigAndModels, refinement_kernel_size: int, **kwargs):
        super().__init__()
