...
def _sleap_analysis_h5_to_diplomat_table(path: tc.PathLike) -> pd.DataFrame:
    ...
    with h5py.File(path, "r") as f:
        ...
        tracks = np.zeros((len(track_inst_counts), len(part_names), 3, frame_count), dtype=np.float32)

        for frame in first_video_frames:
            frame_idx = frame["frame_idx"]
            sub_instances = instances[frame["instance_id_start"]:frame["instance_id_end"]]
            ...
            for instance in sub_instances:
                ...
                sub_points = p_ref[instance["point_id_start"]:instance["point_id_end"]]
        ...
        track_names = [n for i, n in enumerate(track_names) if(track_inst_counts[i] > 0)]
