...
def analyze_frames(
    ...
    **kwargs,
    ...
    if num_outputs is None:
        raise ValueError(
            "'num_outputs' is not set! Please set it to the number of bodies you are tracking."
        )
    else:
        num_outputs = int(num_outputs)

    predictor_cls = get_predictor(
        "SegmentedFramePassEngine" if (predictor is None) else predictor
    )
    print(f"Using predictor: '{predictor_cls.get_name()}'")

    if not predictor_cls.supports_multi_output() and num_outputs > 1:
        raise ValueError(
            f"Predictor '{predictor_cls.get_name()}' doesn't support multiple outputs!"
        )
...
def _analyze_frame_store(
    ...
    if not is_video(frame_store_path):
        raise ValueError(
            "Frame store is missing video data! Please generate frame store with more recent version of diplomat."
        )
    video_path = frame_store_path
    output_path = frame_store_path.parent / (
        frame_store_path.name
        + f".diplomat_{predictor_cls.get_name()}{output_suffix}.csv"
    )
    ...
    with frame_store_path.open("rb") as frame_store:
        with frame_store_fmt.DLFSReader(frame_store) as frame_reader:
                ...
                with Timer() as timer:
                    with TQDMProgressBar(total=num_f) as prog_bar:
                        ...
                        while frame_reader.has_next():
                            batch = frame_reader.read_frames(
                                min(batch_size, num_f - read_frames)
                            )

                            # Fix the batch to only have what is needed...
                            batch.set_source_map(
                                batch.get_source_map()[:, :, :, idx_to_keep]
                            )
                            if batch.get_offset_map() is not None:
                                batch.set_offset_map(
                                    batch.get_offset_map()[:, :, :, idx_to_keep]
                                )
            ...
            p.get_all()[:] = labels
            save_diplomat_table(
                to_diplomat_table(num_outputs, bp_lst, p), str(output_path)
            )
...
def _analyze_single_video(
    ...
    output_path = video_path.parent / (
        video_path.name + f".diplomat_{predictor_cls.get_name()}{output_suffix}.csv"
    )
        ...
        with predictor as pred:
            with Timer() as timer:
                ...
                with TQDMProgressBar(total=num_f - frames_done) as post_pbar:
                    ...
                    poses = pred.on_end(post_pbar)
                    if poses is not None:
                        labels[frames_done : frames_done + poses.get_frame_count()] = (
                            poses.get_all()
                        )
