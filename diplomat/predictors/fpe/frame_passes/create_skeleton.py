...
class CreateSkeleton(FramePass):
    ...
    def run_pass(
        ...
        self._max_locations = [None] * (
            fb_data.metadata.num_outputs * len(fb_data.metadata.bodyparts)
        )
        self._prior_max_locations = None
        ...
        if (lnk_parts is not None) and (lnk_parts != False):
            ...
            for bp1, bp2 in lnk_parts:
                if (bp1 in self._skeleton) and (bp2 in self._skeleton):
                    self._skeleton[bp1, bp2] = Histogram(
                        self.config.bin_size, self.config.bin_offset
                    )
                else:
    ...
    def run_step(
        ...
        if self._current_frame != frame_index:
            ...
            for edge, hist in self._skeleton.items():
                s1, s2 = (
                    edge.node1 * metadata.num_outputs,
                    edge.node2 * metadata.num_outputs,
                )
    ...
    @classmethod
    def cast_skeleton(
        cls, skel: Optional[Dict[str, str]]
    ) -> Union[None, bool, List[str], List[Tuple[str, str]]]:
