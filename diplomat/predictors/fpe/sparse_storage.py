...
from diplomat.utils.extract_frames import (
    pretty_frame_string,
    FrameStringFormats,
    BorderStyle,
)
...
def _next_after_dtype(val, towards, dtype):
    return np.nextafter(
        np.asarray(val).astype(dtype), np.asarray(towards).astype(dtype)
    )
...
class SparseTrackingData:
    ...
    def pack(
        ...
        self._data = np.stack([x_coords, y_coords, probs], axis=0).astype(self.mem_type)
        return self
    ...
    def unpack(self) -> Tuple[Optional[ndarray], Optional[ndarray], Optional[ndarray]]:
        """
    ...
    def desparsify(self, orig_width: int, orig_height: int) -> TrackingData:
        ...
        new_td = TrackingData.empty_tracking_data(
            1, 1, orig_width, orig_height, self._downscaling
        )
        new_td.set_offset_map(
            np.zeros((1, orig_height, orig_width, 1, 2), dtype=np.float32)
        )
    ...
    def sparsify(
        ...
        """
        new_sparse_data = cls(
            track_data.get_down_scaling() if (mode != SparseModes.UPSCALE) else 1
        )
        ...
        if track_data.get_offset_map() is None:
            x_off, y_off = np.zeros(x.shape, dtype=np.float32), np.zeros(
                y.shape, dtype=np.float32
            )
        else:
            x_off, y_off = np.transpose(
                track_data.get_offset_map()[frame, y, x, bodypart]
            )
        ...
        if mode == SparseModes.UPSCALE:
            ...
            np.maximum.at(
                tmp_frame, (fx[in_bounds], fy[in_bounds]), gaussian[in_bounds]
            )
            ...
            if mode == SparseModes.OFFSET_COMBINATION:
                ...
                __, indexes, inverse, counts = np.unique(
                    comb_coords,
                    return_index=True,
                    return_inverse=True,
                    return_counts=True,
                ...
                unique_locs = np.concatenate(
                    [[True], (true_x[:-1] != true_x[1:]) | (true_y[:-1] != true_y[1:])]
                )
    ...
    def to_bytes(self, float_dtype: str) -> bytes:
        ...
        if length == 0:
            return b"".join([enc_length, enc_ds])

        return b"".join([enc_length, enc_ds, self._data.astype(float_dtype).tobytes()])

    def from_bytes_include_length(
        self, float_dtype: str, data: Union[bytes, memoryview]
        ...
        if len(data) < expected_data_size:
            print(
                f"Error: The data buffer length {len(data)} is smaller than the expected size {expected_data_size}."
            )

        # Proceed with the original line, wrapped in a try-except block for safety
        try:
            self._data = np.frombuffer(
                data, float_dtype, length * 3, 4 + float_data_size
            ).reshape((3, length))
            self._downscaling = down_scaling
...
def video_to_sparse_tracking_data_point(
    x: float, y: float, prob: float, downscale: float
):
    return x / downscale, y / downscale, prob


def sparse_tracking_data_to_video_point(
    x: float, y: float, prob: float, downscale: float
):
    return x * downscale, y * downscale, prob
...
class ForwardBackwardFrame:
    ...
    ignore_clustering: bool = (
        False  # Has user edited this cell? If so, disable clustering for this frame...
    )
    disable_occluded: bool = (
        False  # If user has edited this frame, shouldn't try using the hidden state...
    )
    ...
    def entry_to_sparse_track(
        self, field: Literal["frame", "occluded"] = "frame"
    ) -> SparseTrackingData:
        """
    ...
    @staticmethod
    def _save_sparse_track(
        std: Optional[SparseTrackingData], float_dtype: str
    ) -> bytes:
        if std is None:
    ...
    def to_bytes(self, float_dtype: str) -> bytes:
        byte_list = [
            np.asarray(
                [self.ignore_clustering, self.disable_occluded], dtype="?"
            ).tobytes(),
    ...
    @staticmethod
    def _load_array(
        data: bytes, dtype: str, offset: int
        ...
        data = np.frombuffer(data, dtype, size, offset + 4 * (shape_size + 1)).reshape(
            shape
        )
    ...
    def _load_sparse_track(
        data: bytes, float_dtype: str, offset: int
    ) -> Tuple[Optional[SparseTrackingData], int]:
        size_indicator = np.frombuffer(data, "<u4", 1, offset)[0]
        if size_indicator == 0xFFFFFFFF:
            return (None, 4)

        res, length = SparseTrackingData(-1).from_bytes_include_length(
            float_dtype, memoryview(data)[offset:]
        )
    ...
    def from_bytes(self, float_dtype: str, data: bytes) -> "ForwardBackwardFrame":
        self.ignore_clustering, self.disable_occluded = np.frombuffer(
            data, np.dtype("?"), 2
        )
...
def sparse_to_string(data: SparseTrackingData, w: int, h: int, **kwargs) -> str:
    """
    ...
    fmt = (
        *fmt[:2],
        BorderStyle(
            *fmt[2][:8], lambda w, h: f"{extra_info}{type(data).__name__}({w}x{h})"
        ),
    )
