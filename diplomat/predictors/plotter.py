...
class PlotterArgMax(Predictor):
    ...
    def __init__(
        ...
        final_video_name = settings["video_name"].replace("$VIDEO", Path(path).stem)
        self.VIDEO_PATH = str(
    ...
    def _get_offset_map_of(
        self, t: TrackingData, frame: int, bp: int, mode3d: bool = False
    ) -> Optional[tuple]:
    ...
    def _on_frames(self, scmap: TrackingData) -> Pose:
        ...
        for frame in range(scmap.get_frame_count()):
            ...
            for bp, ax in zip(self._part_idx_list, self._axes.flat):
                ...
                ax.set_title(f"Bodypart: {self.bodyparts[bp]}")
